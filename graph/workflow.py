...
from graph.nodes.relevance_check import check_relevance_node, should_process_job
from graph.nodes.resume_generator import generate_resume_node
from graph.nodes.latex_compiler import compile_latex_node
from graph.nodes.sheets_logger import log_to_sheets_node, log_skipped_job_node
...
def route_after_relevance(state: WorkflowState) -> Literal["generate_resume", "log_skipped", "next_job"]:
    """
...
def print_summary_node(state: WorkflowState) -> dict:
    ...
    logger.info(f"Jobs relevant (>={state.get('relevance_threshold', 7)}):   {jobs_relevant}")
...
def run_workflow(
    ...
    """
    from graph.nodes.scheduler import load_config, load_resume_data as load_resume
