...
def check_relevance_node(state: WorkflowState) -> dict[str, Any]:
    ...
    if not description:
        ...
        matching_roles = [role for role in desired_roles if any(word in title_lower for word in role.split() if len(word) > 3)]
        ...
        if matching_core:
            ...
            logger.info(f"Title match score: {score}/10 (core keywords: {matching_core})")
            ...
            return {
                "relevance_result": {
                    ...
                    "missing_requirements": ["Core ML/Data keywords not found in title"],
                },
                "is_relevant": False,
    ...
    try:
        ...
        model = llm_settings.get("model", "anthropic/claude-3.5-sonnet")
